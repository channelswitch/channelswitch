	...
	unsigned e_sz)
{
	...
	int file = creat(tb_filename, 0666);
	if(file < 0) {
		snprintf(e, e_sz, "Could not create %s: %s", tb_filename,
			strerror(errno));
		goto err0;
	}
	...
	status = catf(&everything, BLOB(tb_module),
		inargs,
		outargs,
		module_in,
		module_out,
	...
	if(status < 0) {
		snprintf(e, e_sz, "Could not write %s: %s", tb_filename,
			strerror(errno));
		goto err11;
	}
